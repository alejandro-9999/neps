...
const COLUMNS = [
  {
    title: 'Fecha Prestación del Servicio',
    field: 'cosFechaPrestacionServicio',
    render: renderDate,
  },
  {
    title: 'Fecha de Radicado',
    field: 'cosFechaRadicado',
    render: renderDate,
  },
  {
    title: 'Nit de la IPS',
  ...
  {
    title: 'Razón Social',
    field: 'cosRazonSocialIps',
  },
  {
    title: 'Número de Radicación',
    field: 'cosNumeroRadicado',
  },
  {
    title: 'Factura',
    field: 'cosNumeroFactura',
  },
  {
    title: 'Valor Autorizado para Pago',
    field: 'cosValorAutorizadoPago',
  },
  {
    title: 'Valor Moderadora Eps',
    field: 'cosValorModeradoraEps',
  },
  {
    title: 'Valor Retención',
    field: 'cosValorRetencion',
  },
  {
    title: 'Valor Copago Eps',
    field: 'cosValorCopagoEps',
  },
  {
    title: 'Valor Saldo',
    field: 'cosValorSaldo',
  ...
  {
    title: 'Valor Glosa Pendiente',
    field: 'cosValorGlosaPendiente',
  },
];
...
function renderDescargar(watch) {
  return (
    <div>
      <form target='_blank' method='post' action={Utils.BASE_PATH + 'api/reporte-costos/descargar'}>
        <div className='flex w-full flex-row items-center justify-end gap-8 pt-4'>
          ...
          <div className='flex flex-row items-baseline gap-2'>
            ...
            <input type='hidden' name='numeroRadicado' value={watch('numeroRadicado')} />
...
export default function ReporteCostos() {
  ...
  const onBuscar = useCallback(async (data) => {
    ...
    const resQuery = await axios.post(Utils.BASE_PATH + 'api/reporte-costos', data);
  ...
  const onSubmit = useCallback(async (data) => {
    ...
    if (!data.numeroRadicado || data.numeroRadicado === '') {
      if (
        (!data.fechaInicial || data.fechaInicial === '') &&
        (!data.fechaFinal || data.fechaFinal === '')
      ) {
        setMessageError(
          'Debe ingresar al menos un criterio de búsqueda: (Fecha Inicial, Fecha Final) o (Número de Radicado)',
        );
        return;
      }
    }
    ...
    <div className='flex w-full flex-col '>
      <div className='flex flex-col items-center gap-8'>
        <div className='flex flex-col gap-2'>
          <div className='text-xl font-semibold'>RELACION COSTO FACTURAS</div>
        </div>
                    ...
                    <div class='flex h-9 items-center'>
                      <Controller
                        ...
                        render={({ field }) => (
                          <input
                            {...field}
                            checked={field.value === 'fechaPrestacionServicio'}
                            type='radio'
                            value='fechaPrestacionServicio'
                            class='h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600'
                          />
                        )}
                      />
                      <label for='email' class='ml-3 block leading-6 text-gray-900'>
                        Fecha Prestaci&oacute;n del Servicio
                      </label>
                    </div>
                    <div class='flex h-9 items-center'>
                      <Controller
                        ...
                        render={({ field }) => (
                          <input
                            {...field}
                            checked={field.value === 'fechaRadicacion'}
                            type='radio'
              ...
              <div>
                <div className='flex flex-col gap-2'>
                  <div className='grid grid-cols-2 flex-row items-baseline justify-center gap-2 align-middle'>
                    ...
                    <div>
                      <input
                        {...register('numeroRadicado', { required: false })}
                        type='text'
                        class='block  w-full rounded-md border-0 px-2 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'
                      />
                    </div>
            ...
            <div className='w-full max-w-6xl'>
              <div className='grid grid-cols-2'>
                <div className='flex flex-col gap-1'>
                  <div>
                    <span className='font-medium'>RELACION COSTO FACTURAS</span>
                  </div>
                  <div>
                ...
                <div className='flex flex-col justify-end'>
                  <div className='flex ring-1 ring-gray-200'>
                    <div className='flex-grow bg-sky-200 px-2 font-medium'>
                      T. Valor Autorizado para Pago
                    </div>
                    <div className='p-2'>{totales?.totalAutorizaPago ?? ''}</div>
                  </div>
                  <div className='flex ring-1 ring-gray-200'>
                    <div className='flex-grow bg-sky-200 px-2 font-medium'>
                      T. Valor Moderadora Eps
                    </div>
                    <div className='p-2'>{totales?.totalModeradoraEps ?? ''}</div>
                  </div>
                  <div className='flex ring-1 ring-gray-200'>
                    <div className='flex-grow bg-sky-200 px-2 font-medium'>T. Valor Saldo</div>
                    <div className='p-2'>{totales?.totalValorSaldo ?? ''}</div>
                  </div>
                </div>
              </div>
            </div>
