...
const COLUMNS = [
  {
    ...
    field: 'dglPrefijoFactura',
  },
  {
    title: 'Numero factura',
    field: 'dglNumeroFactura',
  },
  {
    title: 'Nro radicacion de la cuenta',
    field: 'dglNumeroRadicado',
  },
  {
    title: 'Tipo identificacion del afiliado',
    field: 'dglTipoIdentificacionAfiliado',
  },
  {
    title: 'Numero de identificacion del afiliado',
    field: 'dglNumeroIdentificacionAfiliado',
  },
  {
    title: 'Fecha prestacion servicio',
    field: 'dglFechaPrestacionServicio',
    render: renderDate,
  },
  {
    title: 'Fecha de emision de factura',
    field: 'dglFechaEmisionFactura',
    render: renderDate,
  },
  {
    title: 'Fecha presentacion factura',
    field: 'dglFechaPresentacionFactura',
    render: renderDate,
  },
  {
    title: 'Fecha devolucion',
    field: 'dglFechaDevolucion',
    render: renderDate,
  },
  {
    title: 'Valor factura',
    field: 'dglValorFactura',
  },
  {
    title: 'Codigo devolucion',
    field: 'dglCodigoDevolucion',
  },
  {
    title: 'Observacion devolución',
    field: 'dglObservacionDevolucion',
  },
];
  ...
  return (
    <div>
      <form
        target='_blank'
        method='post'
        action={Utils.BASE_PATH + 'api/reporte-devolucion/descargar'}
      >
        <div className='flex w-full flex-row items-center justify-end gap-8 pt-4'>
          ...
          <div className='flex flex-row items-baseline gap-2'>
            ...
            <input type='hidden' name='numeroFactura' value={watch('numeroFactura')} />
...
export default function Devoluciones() {
  ...
  const onBuscar = useCallback(async (data) => {
    ...
    const resQuery = await axios.post(Utils.BASE_PATH + 'api/reporte-devolucion', data);
  ...
  const onSubmit = useCallback(async (data) => {
    ...
    if (!data.numeroFactura || data.numeroFactura === '') {
      if (
        ...
        if (!data.numeroRadicado || data.numeroRadicado === '') {
          setMessageError(
            'Debe ingresar al menos un criterio de búsqueda: (Fecha Inicial, Fecha Final) o (Número de Radicado) o (Número de Factura)',
          );
          return;
        }
      }
    ...
    <div className='flex w-full flex-col '>
      <div className='flex flex-col items-center gap-8'>
        <div className='flex flex-col gap-2'>
          <div className='text-xl font-semibold'>DEVOLUCIONES</div>
        </div>
        <div className='w-full max-w-6xl ring-1 ring-gray-200'>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-2 bg-white p-4'>
            <div className='mt-4 grid grid-cols-2 gap-8'>
              ...
              <div>
                <div className='flex flex-col gap-2'>
                  <label for='email' class='ml-3 block h-6 leading-6 text-gray-900'></label>
                  ...
                  <div className='grid grid-cols-2 flex-row items-baseline justify-center gap-2 align-middle'>
                    ...
                    <div>
                      <input
                        {...register('numeroFactura', { required: false })}
                        type='text'
                        class='block  w-full rounded-md border-0 px-2 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6'
                      />
                    </div>
